...
:subShowAdaptersList
	...
	for /f "skip=3 tokens=1,2,3,*" %%a in ('netsh interface show interface') do (
		...
		if /i "%%b"=="®¤ª«îç¥­" set bNAconn!countNA!=%langON%
		if /i "%%b"=="Connected" set bNAconn!countNA!=%langON%

		if /i "%%b"=="Žâª«îç¥­" (
			set bNAconn!countNA!=%langOFF%
			if /i "%%a"==" §à¥è¥­" set bNAconn!countNA!=%langCABLE%)
	...
	for /l %%x in (1,1,!countNA!) do (
		...
		for /f "usebackq tokens=1-5 delims= " %%a in (`netsh interface ipv4 show addresses name^="!strNAname%%x!"`) do (
			...
			if /i "%%a"=="DHCP" if "%%c"=="„ " set strNAtype%%x=DHCP
			if /i "%%a"=="DHCP" if "%%c"=="¥â" set strNAtype%%x=static
			if /i "%%a"=="DHCP" if "%%c"=="Yes" set strNAtype%%x=DHCP
			if /i "%%a"=="DHCP" if "%%c"=="No" set strNAtype%%x=static
			if /i "%%b"=="è«î§:" set strNAGateway%%x=%%c
			if /i "%%b"=="Gateway:" set strNAGateway%%x=%%c
			if /i "%%a"=="IP- ¤à¥á" set strNAaddress%%x=%%b
...
:LangRUS
	set "langWarn1=^³                       ‚ˆŒ€ˆ….                            ^³"
	set "langWarn2=^³ ‘ªà¨¯â ¡ë« áª®¯¨à®¢ ­ ¢ á¨áâ¥¬­ãî ¤¨à¥ªâ®à¨î ^(%windir%^). ^³"
	set "langWarn3=^³ ’¥¯¥àì § ¯ãáª áªà¨¯â  ¢®§¬®¦¥­ ¨§ «î¡®© ª®¬ ­¤®© áâà®ª¨.   ^³"
	set "langSyntax=‘¨­â ªá¨á"
	set "langHelp1=^/prefix	‡ ¤ ñâ ¬ áªã ¯®¤á¥â¨ ¢ ¢¨¤¥ ¤«¨­ë ¯à¥ä¨ªá  á¥â¨."
	set "langHelp2=auto ¨ dhcp	‚§ ¨¬®§ ¬¥­ï¥¬ë. ‚ª«îç ¥â  ¢â®¬ â¨ç¥áª®¥"
	set "langHelp3= 		­ §­ ç¥­¨ï  ¤à¥á  ®â DHCP-á¥à¢¥à ."
	set "langHelp4=list		‚ë¢®¤¨â á¯¨á®ª á¥â¥¢ëå  ¤ ¯â¥à®¢."
	set "langHelp5=scan		‘ª ­¨àã¥â  ¤à¥á  ¢ ¯®¤á¥â¨."
	set "langHelp6=rename		¥à¥¨¬¥­®¢ë¢ ¥â á¥â¥¢®©  ¤ ¯â¥à."
	set "langHelp7=à¨¬¥àë:"
	set "langSelectOnOff=‚¢¥¤¨â¥ #  ¤ ¯â¥à , çâ®¡ë ¢ª«îç¨âì ¨«¨ ®âª«îç¨âì ¥£® (0 ¤«ï ®â¬¥­ë): "
	set "langSelectRename=‚¢¥¤¨â¥ #  ¤ ¯â¥à , ª®â®àë© á«¥¤ã¥â ¯¥à¥¨¬¥­®¢ âì (0 ¤«ï ®â¬¥­ë): "
	set "langSelectScan=‚¢¥¤¨â¥ #  ¤ ¯â¥à , ¤«ï áª ­¨à®¢ ­¨ï ¥£® ¯®¤á¥â¨ (0 ¤«ï ®â¬¥­ë): "
	set "langSelectApply=‚¢¥¤¨â¥ #  ¤ ¯â¥à  ¤«ï ¯à¨¬¥­¥­¨ï ¯ à ¬¥âà®¢ (0 ¤«ï ®â¬¥­ë): "
	set "langNewName=‚¢¥¤¨â¥ ­®¢®¥ ¨¬ï ¤«ï"
	set "langNow=„«ï"
	set "langDHCP=¢ª«îç¥­ à¥¦¨¬ DHCP."
	set "langHasAddress=­ §­ ç¥­  ¤à¥á"
	set "langAndGate=¨ è«î§"
	set "langTurnOn=‚ª«îç¥­¨¥"
	set "langTurnOff=Žâª«îç¥­¨¥"
	set "langScanFrom=‘ª ­¨à®¢ ­¨¥  ¤à¥á®¢ á"
	set "langTo=¤®"
	set "langChecking=à®¢¥àª "
	set "langFound= ©¤¥­®"
	set "langAnswers=®â¢¥ç ¥â"
	set "langON=‚Š‹ "
	set "langOFF=Ž’Š‹"
	set "langCABLE=Š ¡«"
	set "langTitles=#  ¥¦. ’¨¯	€¤à¥á		Œ áª 		˜«î§		 §¢ ­¨¥"
	exit/b
